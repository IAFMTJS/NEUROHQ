...
export function LearningMonthlyView({ weeks, target }: Props) {
  ...
  return (
    <div className="rounded-xl border border-[var(--card-border)] bg-[var(--bg-primary)]/40 px-4 py-3">
      <p className="mb-2 text-xs font-medium text-[var(--text-muted)]">Weeks this month</p>
          ...
          return (
            <li key={w.weekStart} className="flex items-center gap-2 text-sm">
              <span className="w-24 shrink-0 text-[var(--text-muted)]">
                {new Date(w.weekStart + "T00:00:00").toLocaleDateString("en-GB", { month: "short", day: "numeric" })}
              </span>
              <span className="min-w-[3ch] tabular-nums text-[var(--text-primary)]">{w.minutes}</span>
              <span className="text-[var(--text-muted)]">/ {target} min</span>
              <div className="h-1.5 flex-1 max-w-[80px] overflow-hidden rounded-full bg-[var(--card-border)]">
                <div
