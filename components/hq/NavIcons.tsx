...
/** Rounded, minimal icon set – same size, stroke 1.5, round caps */
const iconClass = "w-6 h-6 shrink-0";
const stroke = 1.5;

function useColor(active: boolean) {
  return active ? "var(--accent-cta)" : "var(--text-muted)";
}

export function IconHQ({ active }: { active: boolean }) {
  const color = useColor(active);
  return (
    <svg className={iconClass} viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth={stroke} strokeLinecap="round" strokeLinejoin="round" aria-hidden>
