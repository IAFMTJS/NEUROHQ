...
const MissionConfirmationModal = dynamic(
  () =>
    import("@/components/dcic/MissionConfirmationModal").then((m) => ({
      default: m.MissionConfirmationModal,
    })),
...
type SuggestedAction =
  ...
  | {
      type: "add_expense";
      label: string;
      payload: { amount_cents: number; date: string; category?: string; note?: string };
    }
  | {
      type: "add_calendar";
      label: string;
      payload: { title: string; start_at: string; end_at: string; sync_to_google?: boolean };
    }
      ...
      >
        ...
        {messages.map((msg) => (
          <div
            ...
            aria-label={
              msg.role === "user" ? "Bericht van jou" : "Bericht van assistant"
            }
          >
            <div
        ...
        {loading && (
          <div className="flex justify-start">
            <div className="max-w-[85%] rounded-2xl rounded-tl-md border border-[var(--card-border)] bg-[var(--bg-surface)] px-4 py-3">
              <p className="text-sm text-[var(--text-muted)]">Analyseren…</p>
            </div>
          </div>
        )}
        ...
        {error && (
          <p
            className="mt-2 text-sm text-[var(--accent-warning)]"
            role="alert"
          >
            {error}
          </p>
