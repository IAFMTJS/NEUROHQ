  ...
  function poolBar(pool: PoolBudget, label: string, accent: string) {
    ...
    return (
      <div key={label} className="flex items-center gap-2">
        <span className="w-14 shrink-0 text-[10px] font-medium text-neuro-muted">{label}</span>
        <div className="h-1.5 flex-1 overflow-hidden rounded-full bg-neuro-border">
        ...
        <span className="w-8 shrink-0 text-right text-[10px] tabular-nums text-neuro-muted">
          {pool.remaining}
        </span>
      </div>
    );
  ...
  return (
    <div className="card-modern-accent overflow-hidden p-0">
      ...
      <div className="p-5 space-y-4">
        <div className="flex items-baseline gap-2">
          <span
            className={`text-3xl font-bold tabular-nums ${
              remaining < 20 ? "text-amber-400" : "text-neuro-blue"
            }`}
          >
            {remaining}
          </span>
          <span className="text-neuro-muted">/ {capacity} headroom</span>
        </div>

        {/* Stacked breakdown */}
        {(segments.length > 0 || remaining > 0) && (
          <div className="space-y-1">
            <p className="text-[10px] font-medium uppercase tracking-wide text-neuro-muted">
              Breakdown
            </p>
            <div className="flex h-3 w-full overflow-hidden rounded-lg bg-neuro-border">
              {segments.map((s) => {
            ...
            <div className="flex flex-wrap gap-x-3 gap-y-0.5 text-[10px] text-neuro-muted">
        ...
        {/* Full stats table */}
        <div className="overflow-x-auto rounded-lg border border-neuro-border/50 bg-neuro-surface/20 px-3 py-2">
          <p className="mb-2 text-[10px] font-medium uppercase tracking-wide text-neuro-muted">
            Full stats (capacity · used · planned · remaining)
          </p>
          <table className="w-full text-left text-xs">
            <thead>
              <tr className="border-b border-neuro-border/50 text-neuro-muted">
            ...
            <tbody className="text-neuro-silver">
              <tr className="border-b border-neuro-border/30">
          ...
          <p className="mt-2 text-[10px] text-neuro-muted">
            Load = headroom before overload. Social/calendar events use more of it. Headroom shown above = min of the three remaining.
          </p>
        </div>

        {/* Per-pool bars */}
        <div className="space-y-2 rounded-lg border border-neuro-border/50 bg-neuro-surface/20 px-3 py-2">
          <p className="text-[10px] font-medium uppercase tracking-wide text-neuro-muted">
