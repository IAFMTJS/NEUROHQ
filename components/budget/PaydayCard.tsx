...
export function PaydayCard({ daysUntilNextIncome, nextPaydayLabel, incomeSources, paydayDayOfMonth, currency = "EUR" }: Props) {
  ...
  return (
    <>
      ...
      <Modal open={showModal} onClose={() => setShowModal(false)} title="Loon en loondag" showBranding>
        <p className="text-sm text-[var(--text-muted)] mb-4">Stel in op welke dag van de maand je loon beschikbaar is. Je kunt ook inkomstenbronnen toevoegen.</p>
        {incomeSources.length === 0 && (
          <>
