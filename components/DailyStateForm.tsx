...
const DIMENSIONS = [
  ...
  { key: "mental_battery" as const, label: "Mental battery" },
...
export function DailyStateForm({ date, initial }: Props) {
  ...
  const [isRestDay, setIsRestDay] = useState(initial.is_rest_day ?? false);
          ...
          {DIMENSIONS.map((d) => (
            <SliderRow
              ...
              value={
                d.key === "energy" ? energy :
                d.key === "focus" ? focus :
                d.key === "mental_battery" ? mentalBattery :
                d.key === "sensory" ? sensory : social
              }
              onChange={
                d.key === "energy" ? setEnergy :
                d.key === "focus" ? setFocus :
                d.key === "mental_battery" ? setMentalBattery :
        ...
        <div className="mt-6 flex flex-wrap items-center gap-4 border-t border-[var(--card-border)]/80 pt-5">
          <label className="flex cursor-pointer items-center gap-2">
            <input
              ...
              className="h-4 w-4 rounded border-[var(--card-border)] text-[var(--accent-focus)] focus:ring-[var(--accent-focus)]"
              aria-label="Rustdag"
            />
            <span className="text-sm font-medium text-[var(--text-muted)]">Rustdag (streak shield)</span>
          </label>
