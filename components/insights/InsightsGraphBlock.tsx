...
export function InsightsGraphBlock({ graphData }: Props) {
  ...
  const dataKey = layer === "xp" ? "xp" : layer === "energy" ? "energy" : "focus";
    ...
    >
      <div className="border-b border-[var(--card-border)] px-4 py-3">
        ...
        <p className="text-sm text-[var(--text-muted)]">
          Laatste 14 dagen. Schakel tussen XP, energie en focus.
        </p>
        <div className="mt-3 flex gap-2">
          {(["xp", "energy", "focus"] as const).map((l) => (
            <button
              key={l}
              type="button"
              onClick={() => setLayer(l)}
            ...
            >
              {l === "xp" ? "XP" : l === "energy" ? "Energie" : "Focus"}
            </button>
          ))}
        </div>
      </div>
