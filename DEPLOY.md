...
**PWA:** Set `NEXT_PUBLIC_APP_URL` to your production URL (e.g. `https://neurohq.vercel.app`) so "Add to home screen" uses an absolute start URL; otherwise reopening the PWA on mobile can show "link not found".
...
- **“Something went wrong” / Server Components error after deploy:** Next.js hides the real error in production. (1) **Check env:** open **https://your-app.vercel.app/api/env-check** — if `ok` is false, add **NEXT_PUBLIC_SUPABASE_URL** and **NEXT_PUBLIC_SUPABASE_ANON_KEY** in Vercel → Project → Settings → Environment Variables (from Supabase → Project Settings → API), then **redeploy**. (2) **See the real error:** Vercel → Project → **Logs**; reproduce the error and look for `[Supabase server]` or the stack trace. (3) If env is set, run all migrations in Supabase SQL Editor (see §1.4).
