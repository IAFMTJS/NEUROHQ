-- Extends admin_platform_diagnostics: signups, admins, push, platform_events, mission_outcome_events, task_events, play tasks.
...
declare
  ...
  since_7d_ts timestamptz := (now() - interval '7 days');
begin
  ...
  return jsonb_build_object(
    ...
    'users', (
      select jsonb_build_object(
        ...
        'signups_last_7d', (select count(*)::int from public.users where created_at >= since_7d_ts),
        'signups_last_30d', (select count(*)::int from public.users where created_at >= since_ts),
        'with_push_subscription', (
          select count(*)::int from public.users where push_subscription_json is not null
        )
      )
    ),
    'tasks', (
    ...
    'platform_events', (
      select jsonb_build_object(
        'total', (select count(*)::int from public.platform_events),
        'active_rows', (select count(*)::int from public.platform_events where active),
        'live_now', (
        ...
        'created_last_30d', (select count(*)::int from public.platform_events where created_at >= since_ts)
      )
    ),
    'mission_outcome_events', (
      select jsonb_build_object(
        'rows_last_30d', (select count(*)::int from public.mission_outcome_events where occurred_at >= since_ts),
        'distinct_users_last_30d', (
          select count(distinct user_id)::int from public.mission_outcome_events where occurred_at >= since_ts
        ),
        'complete_last_30d', (
          select count(*)::int from public.mission_outcome_events
          where occurred_at >= since_ts and outcome = 'complete'
        ),
        'skip_last_30d', (
          select count(*)::int from public.mission_outcome_events
          where occurred_at >= since_ts and outcome = 'skip'
        ),
        'reschedule_last_30d', (
          select count(*)::int from public.mission_outcome_events
          where occurred_at >= since_ts and outcome = 'reschedule'
        ),
        'delete_last_30d', (
          select count(*)::int from public.mission_outcome_events
          where occurred_at >= since_ts and outcome = 'delete'
        )
      )
    ),
    'task_events', (
      select jsonb_build_object(
        'rows_last_30d', (select count(*)::int from public.task_events where occurred_at >= since_ts),
        'distinct_users_last_30d', (
          select count(distinct user_id)::int from public.task_events where occurred_at >= since_ts
        ),
        'start_last_30d', (
          select count(*)::int from public.task_events where occurred_at >= since_ts and event_type = 'start'
        ),
        'complete_last_30d', (
          select count(*)::int from public.task_events where occurred_at >= since_ts and event_type = 'complete'
        ),
        'abandon_last_30d', (
          select count(*)::int from public.task_events where occurred_at >= since_ts and event_type = 'abandon'
        )
      )
    ),
    'play_tasks', (
      select jsonb_build_object(
        'open_fun', (
          select count(*)::int from public.tasks
          where deleted_at is null and not completed and play_kind = 'fun'
        ),
        'open_unwind', (
          select count(*)::int from public.tasks
          where deleted_at is null and not completed and play_kind = 'unwind'
        ),
        'open_challenge', (
          select count(*)::int from public.tasks
          where deleted_at is null and not completed and play_kind = 'challenge'
        )
      )
    ),
    'per_user', coalesce(
...
comment on function public.admin_platform_diagnostics() is
  'Returns JSON platform usage stats for admins (30-day window, per-user snapshot, events, quarter outcomes, push).';
