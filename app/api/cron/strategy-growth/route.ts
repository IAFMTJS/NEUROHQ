...
/**
 * GitHub Actions should call this daily (see `.github/workflows/cron-strategy-growth.yml`).
 * Strategy / Growth nudges: check-in cadence, incomplete quarterly form, growth protocol unset,
 * learning idle, monthly tip. At most one push per user per run.
 */
...
function classifyCheckIn(checkedAt: string | null, createdAt: string): "soft" | "firm" | null {
  const now = Date.now();
  const accountAgeDays = Math.floor((now - new Date(createdAt).getTime()) / (24 * 60 * 60 * 1000));
  ...
  const days = Math.floor((now - new Date(checkedAt).getTime()) / (24 * 60 * 60 * 1000));
