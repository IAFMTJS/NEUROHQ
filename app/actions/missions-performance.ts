...
function computeUMS(
  ...
  const completionProbability = Math.min(1, Math.max(0.2, opts.completionRate));
...
export async function getRecoveryCampaignNeeded(): Promise<{
  ...
  const lastCompletion = (streak as { last_completion_date?: string | null } | null)?.last_completion_date ?? null;
  const todayStr = new Date().toISOString().slice(0, 10);
  if (!lastCompletion) return { needed: true, lastCompletionDate: null, daysInactive: 999 };

  const last = new Date(lastCompletion + "T12:00:00Z");
  const today = new Date(todayStr + "T12:00:00Z");
  const daysInactive = Math.floor((today.getTime() - last.getTime()) / 86400000);
