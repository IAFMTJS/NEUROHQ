...
/** Recompute and upsert user_analytics_daily for a given date from tasks, learning_sessions, daily_state. */
export async function upsertDailyAnalytics(date: string) {
  const supabase = await createClient();
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) return;

  const [tasksRes, learningRes, stateRes] = await Promise.all([
    supabase.from("tasks").select("completed, completed_at, due_date").eq("user_id", user.id).eq("due_date", date),
  ...
  const brainStatusLogged = !!(stateRes.data && (stateRes.data as { energy?: number }).energy != null);

  const { data: existing } = await supabase
    .from("user_analytics_daily")
    .select("active_seconds, carry_over_count")
    .eq("user_id", user.id)
    .eq("date", date)
    .single();

  const carryOver = (existing?.carry_over_count as number | undefined) ?? 0;
  const activeSeconds = (existing?.active_seconds as number | undefined) ?? 0;
