...
export async function getIdentityEngine(): Promise<IdentityEngineState> {
  ...
  const [identityRow, reputationRow, streakRow] = await Promise.all([
    ...
    supabase.from("user_reputation").select("discipline, consistency, impact").eq("user_id", user.id).single(),
  ...
  const repData = reputationRow.data as { discipline?: number; consistency?: number; impact?: number } | null;
  if (repData && (repData.discipline != null || repData.consistency != null || repData.impact != null)) {
    reputation = {
      discipline: repData.discipline ?? 0,
      consistency: repData.consistency ?? 0,
      impact: repData.impact ?? 0,
    };
  } else {
