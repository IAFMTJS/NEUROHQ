...
export async function upsertStrategyFocus(params: {
  ...
  if (existing?.id) {
    await supabase.from("strategy_focus").update(payload).eq("id", existing.id).eq("user_id", user.id);
  } else {
    await supabase.from("strategy_focus").insert(payload);
  }
  revalidatePath("/strategy");
  revalidatePath("/dashboard");
}

/** Set active strategy to inactive (archive). */
export async function archiveStrategyFocus(strategyId: string): Promise<void> {
  const supabase = await createClient();
  const {
    data: { user },
  } = await supabase.auth.getUser();
...
/** XP per domain for a date range (from behaviour_log + missions.domain). */
export async function getXPByDomain(startDate: string, endDate: string): Promise<Record<StrategyDomain, number>> {
  ...
  if (!logs?.length) return out as Record<StrategyDomain, number>;
