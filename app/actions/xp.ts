...
export async function awardXPForTaskComplete(
  ...
  const [alignMult, antiGrindMult, adaptiveMod, primeMult, progressionMult, dangerousCtx, quarterXpMult] =
    await Promise.all([
      ...
      import("./prime-window").then((m) =>
        m.isInsidePrimeWindow(completionDate ? new Date(completionDate) : undefined).then((inside) => (inside ? 1.1 : 1))
      ),
      import("./progression-rank").then((m) => m.getProgressionRankState()).then((s) => (s ? s.xpMultiplier : 1)),
      completionDate
        ? import("./dangerous-modules-context").then((m) => m.getDangerousModulesContext(completionDate))
        : Promise.resolve(null),
      import("./quarter-engine-snapshot").then((m) => m.getQuarterXpMultiplierForUser()),
    ]);
