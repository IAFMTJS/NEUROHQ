...
export function bucketTodayItems(
  ...
  const { streakAtRisk, nearUnlockSkills = [] } = options;
  ...
  for (const item of sorted) {
    ...
    if (streakAtRisk && (item.streakCritical || (critical.length === 0 && high_impact.length === 0))) {
      critical.push(item);
      energyUsed += cost;
    } else if (!wouldExceed && (item.xpReward ?? 0) >= 80 && high_impact.length < 3) {
